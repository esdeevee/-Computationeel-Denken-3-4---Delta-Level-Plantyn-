...
2. Schrijf een functie `x_i(x1, x2, type)` die een waarde teruggeeft in het (deel)interval $$[x1, x2]$$:
    * als `type` gelijk is aan `'LINKS'`, geeft de functie de waarde van `x1` terug;
    * als `type` gelijk is aan `'MIDDEN'`, geeft de functie het midden van $$[x1, x2]$$ terug;
